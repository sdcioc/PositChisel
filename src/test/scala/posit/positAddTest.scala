...
object PositTestAdd {
    def int_add(a : Int, b : Int, size: Int, max_exponent_size : Int) : Int = {
...
class TesterAddPosit(dut : PositAdd) extends PeekPokeTester(dut) {
    var aux: Int = 0;
    ...
    step(1)
    aux = PositTestAdd.int_add(index, jindex, 8, 1)
...
object TesterAddPosit extends App {
    chisel3.iotesters.Driver(() => new PositAdd(1, 8)) { c => new TesterAddPosit(c) }
}