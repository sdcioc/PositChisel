...
class PositMul(es: Int, size : Int) extends Module {
    ...
    //
    val fraction_1 = Wire(UInt(size.W))
    val fraction_2 = Wire(UInt(size.W))
    ...
    fraction_1 := (1.U << (size.U-2.U)) | (io.i_posit_1.fraction << (size.U-2.U-io.i_posit_1.fraction_size))
    ...
    */
    ...
    val fraction_size = Wire(UInt((log2Ceil(size)+1).W))
    ...
    */
    ...
    when (io.i_posit_1.special_number) {
        ...
        when(regime_1_result >= (size.S - 2.S)) {
            ...
            when(io.o_posit.max_exponent_size - exponent_size >= 2.U) {
                ...
                bit_nplus1 := ~((fraction_2_result & (1.U << ((2*size-5).U - fraction_size))) === 0.U) & 1.U
                bits_more := ~((fraction_2_result & (Cat(0.U, Fill((2*size-1), 1.U)) >> (3.U+fraction_size))) === 0.U) & 1.U
                io.o_posit.exponent := exponent_2_result
                io.o_posit.fraction := fraction_2_result >> ((2*size-4).U - fraction_size)
                io.o_posit.exponent_size := exponent_size
    ...
    when (io.o_posit.special_number) {
        ...
        when (io.i_posit_1.sign ^ io.i_posit_2.sign === 1.U) {
