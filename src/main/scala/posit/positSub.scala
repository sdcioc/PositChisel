...
class PositSub(es: Int, size : Int) extends Module {
    ...
    val shiftLeft =  Wire(UInt((log2Ceil(size)+2).W))
    ...
    when(fraction_to_exponent > io.i_posit_1.exponent) {
        exponent_1_result := (1.U << io.i_posit_1.max_exponent_size) + io.i_posit_1.exponent - fraction_to_exponent
        exponent_to_regime := 1.U
    } .otherwise {
        exponent_1_result := io.i_posit_1.exponent - fraction_to_exponent
        exponent_to_regime := 0.U
    }
    exponent_2_result := exponent_1_result
    val regime_1_result = Wire(SInt((size+2).W))
    regime_1_result := io.i_posit_1.regime - exponent_to_regime.zext - fraction_to_regime.zext
    ...
    */
    ...
    when (io.i_posit_1.special_number) {
        when (io.i_posit_1.sign) {
            ...
            when (io.i_posit_2.special_number) {
                ...
                io.o_posit.sign := io.i_posit_2.sign ^ initial_sign
        ...
        when (io.i_posit_2.sign) {
            ...
            io.o_posit.sign := io.i_posit_1.sign ^ initial_sign
            io.o_posit.special_number := io.i_posit_1.special_number
            io.o_posit.regime := io.i_posit_1.regime
            io.o_posit.exponent := io.i_posit_1.exponent
            io.o_posit.fraction := io.i_posit_1.fraction
    ...
    } .elsewhen (fraction_1_result === 0.U) {
        io.o_posit.sign := 0.U
        io.o_posit.special_number := 1.U
        io.o_posit.regime := (-(size-2)).S
        io.o_posit.exponent := 0.U
