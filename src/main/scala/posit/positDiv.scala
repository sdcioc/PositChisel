...
class PositDiv(es: Int, size : Int) extends Module {
    ...
    */
    ...
    when(io.i_posit_2.exponent > io.i_posit_1.exponent) {
        ...
        when(fraction_to_exponent > exponent_1_result) {
            ...
            exponent_2_result := exponent_1_result - fraction_to_exponent
            exponent_to_regime := 1.U
        }
    } .otherwise {
        exponent_1_result := io.i_posit_1.exponent - io.i_posit_2.exponent
        when(fraction_to_exponent  > exponent_1_result) {
            exponent_2_result := (1.U << io.i_posit_1.max_exponent_size) + exponent_1_result - fraction_to_exponent
            exponent_to_regime := 1.U
        } .otherwise {
    ...
    when (io.i_posit_1.special_number) {
        ...
        when(regime_1_result >= (size.S - 2.S)) {
            ...
            when(io.o_posit.max_exponent_size - exponent_size >= 2.U) {
                ...
                bits_more := (~( (exponent_2_result & (aux_1 >> 1)) === 0.U) & 1.U) | (~(fraction_2_result === 0.U) & 1.U)  | (~(fraction_1_result_rem === 0.U) & 1.U)
                ...
                bits_more := (~(fraction_2_result === 0.U) & 1.U) | (~(fraction_1_result_rem === 0.U) & 1.U)
