...
class PositDivider (ps : Int) extends Module {
    ...
    when( (io.i_posit_1.sign & io.i_posit_1.special_number) === 1.U ) {
        ...
        when(io.i_posit_2.exponent > io.i_posit_1.exponent) {
            io.o_posit.exponent := io.i_posit_1.exponent + (1.U <<  io.i_posit_1.exponent_size) - io.i_posit_2.exponent
            io.o_posit.regime := io.i_posit_1.regime - io.i_posit_2.regime - 1.S
        } .otherwise {
            io.o_posit.exponent := io.i_posit_1.exponent - io.i_posit_2.exponent
            io.o_posit.regime := io.i_posit_1.regime - io.i_posit_2.regime
        }

        io.o_posit.fraction_size := (io.i_posit_1.fraction_size + ps.U) - io.i_posit_2.fraction_size
        io.o_posit.fraction := (io.i_posit_1.fraction << ps) / io.i_posit_2.fraction
        io.o_posit.b_m := (( (io.i_posit_1.fraction << ps) % io.i_posit_2.fraction) =/= 0.U)
    }
}

class PositDividerTester (ps : Int) extends Module {
    val io = IO(new Bundle {
    ...
    val divider =  Module(new PositDivider(ps))
    ...
    divider.io.i_posit_1 := decoder_1.io.o_posit
    divider.io.i_posit_2 := decoder_2.io.o_posit
    val encoder =  Module(new EncodePosit(ps))
    encoder.io.i_posit := divider.io.o_posit
    ...
    io.debug_fraction_2 := divider.io.o_posit.fraction
    io.debug_exponent_2 := divider.io.o_posit.exponent
    io.debug_regime_2 := divider.io.o_posit.regime
