...
class PositSQRT (ps : Int) extends Module {
    ...
    val shiftleft = Wire(UInt(2.W))
    shiftleft := (io.i_posit.fraction_size & 1.U) + (io.i_posit.exponent & 1.U)
    sqrt_module.io.i_bits := io.i_posit.fraction << shiftleft
    ...
    when( io.i_posit.special_number === 1.U ) {
        ...
        io.o_posit.b_m := (sqrt_module.io.o_R =/= 0.U)
    }
}

class PositSQRTTester (ps : Int) extends Module {
    val io = IO(new Bundle {
    ...
    val sqrt_module =  Module(new PositSQRT(ps))
    val decoder =  Module(new DecodePosit(ps))
    decoder.io.i_bits := io.i_posit
    decoder.io.i_es := io.i_es
    sqrt_module.io.i_posit := decoder.io.o_posit
    sqrt_module.io.i_ready := io.i_ready
    val encoder =  Module(new EncodePosit(ps))
    encoder.io.i_posit := sqrt_module.io.o_posit
    io.o_posit := encoder.io.o_bits
    io.o_ready := sqrt_module.io.o_ready



    io.debug_fraction :=  sqrt_module.io.o_posit.fraction
    io.debug_exponent :=  sqrt_module.io.o_posit.exponent
    io.debug_regime :=  sqrt_module.io.o_posit.regime
    io.debug_b_nplus1 := encoder.io.debug_b_nplus1
    io.debug_b_m := encoder.io.debug_b_m
    io.debug_addOne := encoder.io.debug_addOne
    io.debug_fs := sqrt_module.io.o_posit.fraction_size
}
